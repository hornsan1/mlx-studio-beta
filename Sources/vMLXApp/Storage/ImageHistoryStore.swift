...
struct ImageGenerationMetadataSidecar: Codable {
    ...
    var createdAt: Date
    var durationMs: Int?
    var status: ImageGenerationRecord.Status
    var runtimeName: String?
    var modelPath: String?
    var runtimeProofPath: String?
    var runtimeProof: ImageRuntimeProofStore.Proof?
}

extension ImageGenerationRecord {
    enum MetadataSidecarStatus: Equatable {
        case missing
        case saved
        case stale(String)
    }

    var metadataSidecarURL: URL? {
        guard let outputPath else { return nil }
        return URL(fileURLWithPath: outputPath).appendingPathExtension("metadata.json")
    }

    var metadataSidecarStatus: MetadataSidecarStatus {
        guard let url = metadataSidecarURL,
              FileManager.default.fileExists(atPath: url.path)
        else { return .missing }
        guard let data = try? Data(contentsOf: url),
              let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
        else { return .stale("Unreadable JSON") }
        ...
        for (key, value) in expected {
            guard object[key] as? String == value else {
                return .stale("Mismatched \(key)")
            }
        }
        guard object["schemaVersion"] as? Int == 1 else {
            return .stale("Mismatched schema")
        }
        return .saved
    }

    func metadataSidecar(
        runtimeName: String? = nil,
        modelPath: String? = nil
    ) -> ImageGenerationMetadataSidecar {
        let settings = try? JSONDecoder().decode(
            ImageGenSettings.self,
            from: Data(settingsJSON.utf8)
        )
        let proof = runtimeName.flatMap {
            ImageRuntimeProofStore.latestProof(runtimeName: $0)
        }
        return ImageGenerationMetadataSidecar(
            id: id,
            modelAlias: modelAlias,
            prompt: prompt,
            ...
            modelPath: modelPath,
            runtimeProofPath: runtimeName.map { ImageRuntimeProofStore.proofURL(runtimeName: $0).path },
            runtimeProof: proof
        )
    }

    func metadataExportFilename() -> String {
        "\(Self.fileSafe(modelAlias))-\(id.uuidString.prefix(8)).json"
    }

    func metadataExportData(
        runtimeName: String? = nil,
        modelPath: String? = nil
    ) throws -> Data {
        ...
        return try encoder.encode(
            metadataSidecar(runtimeName: runtimeName, modelPath: modelPath)
        )
    }

    @discardableResult
    func writeMetadataExport(
        to directory: URL,
        runtimeName: String? = nil,
        modelPath: String? = nil
    ) throws -> URL {
        ...
        let url = directory.appendingPathComponent(metadataExportFilename())
        try metadataExportData(runtimeName: runtimeName, modelPath: modelPath)
    ...
    func writeMetadataSidecar(
        ...
        guard let url = metadataSidecarURL else { return nil }
        try metadataExportData(runtimeName: runtimeName, modelPath: modelPath)
            .write(to: url, options: .atomic)
        return url
    }

    func deleteOutputAndSidecar() {
        if let sidecar = metadataSidecarURL {
    ...
    private static func fileSafe(_ title: String) -> String {
        ...
        return cleaned.isEmpty ? "MLX Studio Image" : cleaned
    }
}
