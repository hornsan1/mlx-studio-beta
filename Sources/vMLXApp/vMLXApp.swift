...
struct MLXStudioApp: App {
    ...
    var body: some Scene {
        ...
        .commands {
            ...
            // wired to the native Studio chat workspace so Cmd-N from any tab
...
enum StudioChatCommandKind: Equatable {
    case newSession
...
final class AppState {
    ...
    private func refreshAppNapActivity() {
        ...
        if anyActive, appNapActivityToken == nil {
            appNapActivityToken = ProcessInfo.processInfo.beginActivity(
                ...
                reason: "MLX Studio model is loading or generating"
            )
    ...
    var selectedStudioChatSessionID: UUID? = nil
    var lastClosedStudioChatSessionID: UUID? = nil
    ...
    func requestStudioChatCommand(_ command: StudioChatCommandKind) {
        mode = .chat
    ...
    /// First-launch wizard shown when MLX Studio onboarding has not completed.
    var showSetup: Bool = !UserDefaults.standard.bool(forKey: ExperienceMode.onboardingCompleteKey)
    func markFirstLaunchComplete(mode selectedMode: ExperienceMode? = nil) {
        if let selectedMode {
            setExperienceMode(selectedMode)
        }
        UserDefaults.standard.set(true, forKey: ExperienceMode.onboardingCompleteKey)
        showSetup = false
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.xs) {
            ...
            Picker("Experience", selection: Binding(
                get: { appState.experienceMode },
                set: { appState.setExperienceMode($0) }
            )) {
