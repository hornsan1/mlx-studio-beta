...
struct ImageSettingsInlinePanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            HStack(spacing: Theme.Spacing.sm) {
                ...
                .buttonStyle(.plain)
                .help(L10n.ImageUI.saveAsDefault.render(appLocale))
            }
            ...
            sliderRow(
                ...
                range: 0...20,
                step: 0.1,
                display: String(format: "%.1f", settings.guidance)
            )

            HStack(spacing: Theme.Spacing.sm) {
                dimensionStepper("W", value: $settings.width)
                dimensionStepper("H", value: $settings.height)
            }

            HStack(spacing: Theme.Spacing.sm) {
                stepperChip("Count", value: $settings.numImages, range: 1...4)
                seedField
            }
    ...
    private func dimensionStepper(
        _ label: String,
        value: Binding<Int>
    ) -> some View {
        Stepper(value: value, in: 256...ImageGenerationSafety.maximumDimension, step: 64) {
    ...
    private var seedField: some View {
        HStack(spacing: 6) {
            ...
            .buttonStyle(.plain)
            .help(L10n.ImageUI.random.render(appLocale))
        }
        .padding(.horizontal, Theme.Spacing.sm)
        .padding(.vertical, 7)
        .background(Theme.Colors.surfaceHi)
    ...
    private var memoryEstimate: some View {
        let estimate = ImageGenerationSafety.estimate(
            settings: settings,
            modelStorageBytes: modelStorageBytes
        )
        let warning = ImageGenerationSafety.validationMessage(
            settings: settings,
            modelStorageBytes: modelStorageBytes
        )
        return VStack(alignment: .leading, spacing: 3) {
            Label("Unified memory", systemImage: "memorychip")
                .font(Theme.Typography.caption)
                .foregroundStyle(Theme.Colors.textMid)
            Text("\(estimate.summary) - \(estimate.budgetSummary)")
                .font(Theme.Typography.caption)
                .foregroundStyle(Theme.Colors.textLow)
            if let warning {
                Text(warning)
...
struct ImageSettingsDrawer: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            ...
            row("Width", value: Double(settings.width), range: 256...Double(ImageGenerationSafety.maximumDimension), step: 64) { v in
                settings.width = Int(v)
            }
            row("Height", value: Double(settings.height), range: 256...Double(ImageGenerationSafety.maximumDimension), step: 64) { v in
