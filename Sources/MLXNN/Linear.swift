...
open class Linear: Module, UnaryLayer, Quantizable {
    ...
    public init(_ inputDimensions: Int, _ outputDimensions: Int, bias: Bool = true) {
        ...
        self.weight = MLXNNRandomInit.uniform(-scale ..< scale, [outputDimensions, inputDimensions])
        if bias {
            self.bias = MLXNNRandomInit.uniform(-scale ..< scale, [outputDimensions])
        } else {
            self.bias = nil
        }
        super.init()
...
open class Bilinear: Module {
    ...
    public init(
        ...
        self.weight = MLXNNRandomInit.uniform(
