...
public final class FluxModulation: Module {
    ...
    public init(dim: Int, doubleMode: Bool) {
        ...
        self._linear.wrappedValue = Linear(dim, dim * numMods)
        super.init()
    }
...
public final class FluxDoubleStreamBlock: Module {
    ...
    @ModuleInfo public var imgAttnQKV: Linear
    public let imgAttnNorm: QKNorm
    @ModuleInfo public var imgAttnProj: Linear
    public let imgNorm2: LayerNorm
    @ModuleInfo public var imgMlp0: Linear
    @ModuleInfo public var imgMlp2: Linear

    // Text stream
    public let txtMod: FluxModulation
    public let txtNorm1: LayerNorm
    @ModuleInfo public var txtAttnQKV: Linear
    public let txtAttnNorm: QKNorm
    @ModuleInfo public var txtAttnProj: Linear
    public let txtNorm2: LayerNorm
    @ModuleInfo public var txtMlp0: Linear
    @ModuleInfo public var txtMlp2: Linear
    ...
    public init(dim: Int, numHeads: Int, mlpRatio: Float = 4.0) {
        ...
        self._imgAttnQKV.wrappedValue = Linear(dim, dim * 3)
        self.imgAttnNorm = QKNorm(headDim: headDim)
        self._imgAttnProj.wrappedValue = Linear(dim, dim)
        self.imgNorm2 = LayerNorm(dimensions: dim, eps: 1e-6, affine: false)
        self._imgMlp0.wrappedValue = Linear(dim, mlpDim)
        self._imgMlp2.wrappedValue = Linear(mlpDim, dim)
        ...
        self._txtAttnQKV.wrappedValue = Linear(dim, dim * 3)
        self.txtAttnNorm = QKNorm(headDim: headDim)
        self._txtAttnProj.wrappedValue = Linear(dim, dim)
        self.txtNorm2 = LayerNorm(dimensions: dim, eps: 1e-6, affine: false)
        self._txtMlp0.wrappedValue = Linear(dim, mlpDim)
        self._txtMlp2.wrappedValue = Linear(mlpDim, dim)
...
public final class FluxSingleStreamBlock: Module {
    ...
    @ModuleInfo public var linear1: Linear  // projects to qkv + mlp_in concatenated
    public let qkNorm: QKNorm
    @ModuleInfo public var linear2: Linear  // projects attn + mlp concatenated back to dim
    ...
    public init(dim: Int, numHeads: Int, mlpRatio: Float = 4.0) {
        ...
        self._linear1.wrappedValue = Linear(dim, dim * 3 + mlpDim)
        self.qkNorm = QKNorm(headDim: headDim)
        // linear2 takes (attn: dim) + (mlp: mlpDim) back to dim
        self._linear2.wrappedValue = Linear(dim + mlpDim, dim)
...
public final class FluxFinalLayer: Module {
    public let norm: LayerNorm
    @ModuleInfo public var linear: Linear
    @ModuleInfo public var mod: Linear

    public init(dim: Int, patchSize: Int = 2, outChannels: Int = 16) {
        self.norm = LayerNorm(dimensions: dim, eps: 1e-6, affine: false)
        self._linear.wrappedValue = Linear(dim, patchSize * patchSize * outChannels)
        // Modulation: produces (shift, scale) from vec.
        self._mod.wrappedValue = Linear(dim, dim * 2)
        super.init()
    }
...
public final class FluxDiTModel: Module {
    ...
    @ModuleInfo public var imgIn: Linear   // patch embed: (patch² * in_channels) → dim
    @ModuleInfo public var timeIn0: Linear
    @ModuleInfo public var timeIn2: Linear
    @ModuleInfo public var vectorIn0: Linear  // pooled CLIP projection
    @ModuleInfo public var vectorIn2: Linear
    @ModuleInfo public var guidanceIn0: Linear?
    @ModuleInfo public var guidanceIn2: Linear?
    @ModuleInfo public var txtIn: Linear      // T5 projection
    ...
    public init(config: FluxDiTConfig) {
        ...
        self._imgIn.wrappedValue = Linear(
            config.patchSize * config.patchSize * config.inChannels,
            config.dim
        )
        self._timeIn0.wrappedValue = Linear(256, config.dim)
        self._timeIn2.wrappedValue = Linear(config.dim, config.dim)
        self._vectorIn0.wrappedValue = Linear(768, config.dim)   // pooled CLIP dim
        self._vectorIn2.wrappedValue = Linear(config.dim, config.dim)
        if config.guidanceEmbed {
            self._guidanceIn0.wrappedValue = Linear(256, config.dim)
            self._guidanceIn2.wrappedValue = Linear(config.dim, config.dim)
        } else {
            self._guidanceIn0.wrappedValue = nil
            self._guidanceIn2.wrappedValue = nil
        }
        self._txtIn.wrappedValue = Linear(config.textDim, config.dim)
