...
public final class VAEAttnBlock: Module {
    ...
    public init(channels: Int) {
        ...
        self._q.wrappedValue = Linear(channels, channels)
        self._k.wrappedValue = Linear(channels, channels)
        self._v.wrappedValue = Linear(channels, channels)
        self._proj.wrappedValue = Linear(channels, channels)
...
public enum VAEWeightRemap {
    ...
    public static func remapKey(_ key: String) -> String {
        var k = key
        if k.hasPrefix("vae.") {
            k.removeFirst("vae.".count)
        }
        if k.hasPrefix("decoder.") {
            k.removeFirst("decoder.".count)
        }

        if k.hasPrefix("conv_in.conv2d.") {
            return "convIn." + String(k.dropFirst("conv_in.conv2d.".count))
        }
        if k.hasPrefix("conv_norm_out.norm.") {
            return "normOut." + String(k.dropFirst("conv_norm_out.norm.".count))
        }
        if k.hasPrefix("conv_out.conv2d.") {
            return "convOut." + String(k.dropFirst("conv_out.conv2d.".count))
        }

        if k.hasPrefix("mid_block.resnets.0.") {
            return remapResnet(
                "midResnet1." + String(k.dropFirst("mid_block.resnets.0.".count))
            )
        }
        if k.hasPrefix("mid_block.resnets.1.") {
            return remapResnet(
                "midResnet2." + String(k.dropFirst("mid_block.resnets.1.".count))
            )
        }
        if k.hasPrefix("mid_block.attentions.0.") {
            var rest = String(k.dropFirst("mid_block.attentions.0.".count))
            rest = rest.replacingOccurrences(of: "group_norm.", with: "norm.")
            rest = rest.replacingOccurrences(of: "to_q.", with: "q.")
            rest = rest.replacingOccurrences(of: "to_k.", with: "k.")
            rest = rest.replacingOccurrences(of: "to_v.", with: "v.")
            rest = rest.replacingOccurrences(of: "to_out.0.", with: "proj.")
            return "midAttn." + rest
        }

        if k.hasPrefix("up_blocks.") {
            let rest = String(k.dropFirst("up_blocks.".count))
            let parts = rest.split(separator: ".", maxSplits: 3, omittingEmptySubsequences: false)
            guard parts.count == 4 else { return k }
            let stage = parts[0]
            if parts[1] == "resnets" {
                return remapResnet("upBlocks.\(stage).\(parts[2]).\(parts[3])")
            }
            if parts[1] == "upsamplers", parts[2] == "0" {
                return "upsamples.\(stage).\(parts[3])"
            }
        }
    ...
    private static func remapResnet(_ key: String) -> String {
        key.replacingOccurrences(of: "conv_shortcut.", with: "convShortcut.")
    }
}
