...
public enum LatentSpace {
    ...
    public static func initialNoise(
        ...
        if ProcessInfo.processInfo.environment["VMLX_FLUX_SWIFT_LATENTS"] == "1" {
    ...
    static func deterministicNoise(
        width: Int,
        height: Int,
        layout: LatentLayout,
        batchSize: Int,
        ...
        let shape = noiseShape(width: width, height: height, layout: layout, batchSize: batchSize)
        let count = shape.reduce(1, *)
        let state = seed == 0 ? 0x9E37_79B9_7F4A_7C15 : seed
        let values = deterministicNoiseValues(count: count, seed: state)
        return MLXArray(values).reshaped(shape)
    }

    static func noiseShape(
        width: Int,
        height: Int,
        layout: LatentLayout,
        batchSize: Int
        ...
        switch layout {
        case .fluxPatchified(let channels):
            return [batchSize, latentH * latentW, channels]
        case .spatial(let channels):
            return [batchSize, channels, latentH, latentW]
        }
    }

    static func deterministicNoiseValues(count: Int, seed: UInt64) -> [Float] {
        var state = seed
        return (0..<count).map { _ -> Float in
            state = state &* 6364136223846793005 &+ 1442695040888963407
            let mantissa = UInt32(truncatingIfNeeded: state >> 40) & 0x00FF_FFFF
