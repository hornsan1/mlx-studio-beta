...
public final class T5SelfAttention: Module {
    ...
    @ModuleInfo public var relativeAttentionBias: Embedding   // (32 buckets, 64 heads)
    public let dim: Int
    ...
    public init(dim: Int = 4096, numHeads: Int = 64, headDim: Int = 64) {
        ...
        self._relativeAttentionBias.wrappedValue = Embedding(embeddingCount: 32, dimensions: numHeads)
        super.init()
    }
...
public final class T5DenseReluDense: Module {
    @ModuleInfo public var wi0: Linear   // mflux: wi_0 (gate)
    @ModuleInfo public var wi1: Linear   // mflux: wi_1 (linear)
    @ModuleInfo public var wo: Linear

    public init(dim: Int = 4096, ffnDim: Int = 10240) {
        self._wi0.wrappedValue = Linear(dim, ffnDim, bias: false)
        self._wi1.wrappedValue = Linear(dim, ffnDim, bias: false)
        self._wo.wrappedValue = Linear(ffnDim, dim, bias: false)
        super.init()
    }
...
public final class T5XXLEncoder: Module, TextEncoder, @unchecked Sendable {
    @ModuleInfo public var shared: Embedding   // 32128 × 4096
    ...
    public init(
        ...
        self._shared.wrappedValue = Embedding(embeddingCount: vocabSize, dimensions: hiddenSize)
...
public enum T5XXLWeightRemap {
    ...
    public static func remapKey(_ key: String) -> String {
        ...
        // final_layer_norm.weight / encoder.final_layer_norm.weight → finalLayerNorm.weight
        if k == "final_layer_norm.weight" || k == "encoder.final_layer_norm.weight" {
            return "finalLayerNorm.weight"
        }

        // mflux/MLX FLUX.1 snapshots use `t5_blocks.{i}` rather than HF
        // `encoder.block.{i}`.
        if k.hasPrefix("t5_blocks.") {
            k.replaceSubrange(k.startIndex..<k.index(k.startIndex, offsetBy: "t5_blocks.".count), with: "blocks.")
            return normalizeMappedKey(k)
        }
        ...
        if k.hasPrefix("encoder.block.") {
            ...
            if parts.count == 4,
               ...
               let j = Int(parts[2]) {
                ...
                return normalizeMappedKey("blocks.\(i).\(kind).\(rest)")
            }
        }
    ...
    private static func normalizeMappedKey(_ key: String) -> String {
        var k = key
        k = k.replacingOccurrences(of: "relative_attention_bias", with: "relativeAttentionBias")
        k = k.replacingOccurrences(of: "layer_norm", with: "layerNorm")
        k = k.replacingOccurrences(of: "wi_0", with: "wi0")
        k = k.replacingOccurrences(of: "wi_1", with: "wi1")
        return k
    }
}
