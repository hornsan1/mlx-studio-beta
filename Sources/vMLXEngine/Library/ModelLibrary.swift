...
public actor ModelLibrary {
    ...
    public init(database: ModelLibraryDB) {
        ...
        // Packaged MLX Studio betas may ship a starter model under
        // Contents/Resources/Models. Force the first scan to walk the bundle
        // even when the persistent DB was populated recently by a previous app.
        if !Self.bundledModelRoots().isEmpty {
            pendingForcedRescan = true
        }
    }
    ...
    public func scan(force: Bool = false) async -> [ModelEntry] {
        ...
        // Shard-fingerprint → already-emitted entry. Bundled app resources
        // win first so a self-contained beta prefers its read-only starter
        // model over a duplicate HF cache copy. HF cache then wins over user
        // dirs to avoid duplicate rows for mirrored downloads.
        ...
        for root in Self.bundledModelRoots() {
            for url in walkUserDir(root, maxDepth: 3) {
                passes.append((url, .bundled(root)))
            }
        }
    ...
    // ~/.cache/huggingface/hub, ~/.mlxstudio/models, a configured user dir,
    // or the app's read-only bundled model resources. The destructive delete
    // path intentionally keeps its stricter roots and does not include bundle
    // resources.
    public func isPathUnderKnownRoots(_ url: URL) -> Bool {
        let roots: [URL] = {
            ...
            r.append(contentsOf: Self.bundledModelRoots())
    ...
    private static func bundledModelRoots() -> [URL] {
    ...
    private func walkUserDir(_ root: URL, maxDepth: Int) -> [URL] {
        ...
        while let (dir, depth) = queue.popLast() {
            ...
            // ship a root-level `model_index.json` (Flux, SD, Z-Image) or
            // HF-style component folders (transformer/text_encoder/vae)
            // instead of `config.json`. Accept all loadable markers here;
            // buildEntry() classifies image-modality from the same signals.
            ...
            let hasImageLayout = Self.hasImageRuntimeLayout(at: dir)
            if hasConfig || hasModelIndex || hasImageLayout {
                ...
                let parentIsPipeline = fm.fileExists(
                    ...
                    || Self.hasImageRuntimeLayout(at: parent)
    ...
    private func buildEntry(dir: URL, source: Source, now: Date) -> ModelEntry? {
        ...
        var family = caps.family == "unknown" ? caps.modelType : caps.family
        ...
        let hasRootModelIndex = FileManager.default.fileExists(
            atPath: dir.appendingPathComponent("model_index.json").path)
        let hasImageRuntimeLayout = Self.hasImageRuntimeLayout(at: dir)
        if hasRootModelIndex || hasImageRuntimeLayout {
            modality = .image
        }
        ...
        let pathHint = dir.path.lowercased()
        if pathHint.contains("flux")
            || pathHint.contains("z-image")
            || pathHint.contains("sdxl")
            || pathHint.contains("schnell") {
            modality = .image
        }

        let displayName = deriveDisplayName(dir: dir, source: source, config: json)
        if modality == .image,
           let runtimeName = Self.imageRuntimeNameHint(in: "\(displayName) \(dir.path)")
        {
            family = runtimeName
        }
    ...
    private func deriveDisplayName(
        ...
        if case .bundled(let root) = source {
            ...
            if !last.isEmpty, !looksLikeHash(last) {
                return last
            }
        }
    ...
    private static func hasLoadableModelMarker(at dir: URL) -> Bool {
        let fm = FileManager.default
        return fm.fileExists(atPath: dir.appendingPathComponent("config.json").path)
            || fm.fileExists(atPath: dir.appendingPathComponent("model_index.json").path)
    ...
    internal static func hasImageRuntimeLayout(at dir: URL) -> Bool {
        let hasTransformer = hasWeightFile(
            ...
            under: dir.appendingPathComponent("unet", isDirectory: true)
        )
        let hasTextEncoder = hasWeightFile(
            under: dir.appendingPathComponent("text_encoder", isDirectory: true)
        ) || hasWeightFile(
            under: dir.appendingPathComponent("text_encoder_2", isDirectory: true)
        ) || hasWeightFile(
            under: dir.appendingPathComponent("text_encoder_3", isDirectory: true)
        )
        let hasVAE = hasWeightFile(
            under: dir.appendingPathComponent("vae", isDirectory: true)
        )
        let hasTokenizer =
            fileExists(dir.appendingPathComponent("tokenizer/tokenizer.json"))
            || fileExists(dir.appendingPathComponent("tokenizer_2/tokenizer.json"))
            || fileExists(dir.appendingPathComponent("text_encoder/tokenizer.json"))
            || fileExists(dir.appendingPathComponent("text_encoder_2/tokenizer.json"))

        return hasTransformer && hasTextEncoder && hasVAE && hasTokenizer
    }

    private static func hasWeightFile(under dir: URL) -> Bool {
        let fm = FileManager.default
        ...
        return files.contains { url in
            ...
            guard ext == "safetensors" || ext == "bin" || ext == "gguf" else {
                return false
            }
            return fm.fileExists(atPath: url.resolvingSymlinksInPath().path)
        }
    }

    private static func fileExists(_ url: URL) -> Bool {
        FileManager.default.fileExists(atPath: url.resolvingSymlinksInPath().path)
    }

    internal static func imageRuntimeNameHint(in text: String) -> String? {
