...
public actor Engine {
    ...
    public struct LoadOptions: Sendable {
        ...
        // lands; storage round-trip tests pass independently.
    ...
    public func generateImage(request: [String: Any]) async throws -> [String: Any] {
        ...
        for i in 0..<n {
            ...
            let runtimeName = await (fluxBackend as? FluxEngine)?.lastLoadedName ?? model
            let verification = try ImageRuntimeProofStore.recordVerifiedPNG(
                runtimeName: runtimeName,
                modelPath: lastImageModelPath?.path ?? "",
                outputURL: url,
            ...
            var entry: [String: Any] = [
                "seed": perSettings.seed,
                "proof": [
                    "runtime": runtimeName,
                    "width": verification.width,
                    "height": verification.height,
                    "pixel_variance": verification.pixelVariance,
                ],
            ]
...
public enum EngineError: Error, CustomStringConvertible {
    ...
    /// Image generation needs a working Metal compiler/pipeline service
    /// before model load. If the macOS Metal compiler service is unavailable,
    /// refuse early with a recoverable error instead of letting FLUX load or
    /// generation crash the process.
    case metalRuntimeUnavailable(String)
    ...
    public var description: String {
        ...
        case .metalRuntimeUnavailable(let msg):
            return "Metal image runtime unavailable: \(msg)"
        }
    }
}
