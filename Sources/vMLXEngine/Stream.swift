...
extension Engine {
    ...
    private func performOneGenerationPass(
        ...
        if ProcessInfo.processInfo.environment["VMLX_PROMPT_TRACE"] == "1" {
            let capText: String
            if let caps {
                capText = "family=\(caps.family) modelType=\(caps.modelType) thinkInTemplate=\(caps.thinkInTemplate) source=\(caps.detectionSource.rawValue)"
            } else {
                capText = "nil"
            }
            let pathText = self.loadedModelPath?.path ?? "nil"
            FileHandle.standardError.write(Data(
                "[prompt-trace] loadedModelPath=\(pathText) caps=\(capText) modelStampsThink=\(modelStampsThink) effectiveThinking=\(effectiveThinking)\n".utf8))
        }
        ...
        //
        // MLX Studio beta correctness gate: prompt cache restore/store is
        // opt-in, and only for plain KV today. The sync path now stores a
        // prompt-only snapshot before decode mutates cache state, but
        // persistent TurboQuant/quantized restore is not yet proven by the
        // beta smoke matrix. Keep the older A/B killswitch, and require
        // VMLX_ENABLE_PROMPT_CACHE=1 for deliberate cache testing.
        let promptCacheRequested =
            ProcessInfo.processInfo.environment["VMLX_ENABLE_PROMPT_CACHE"] == "1"
        let promptCacheCompatible =
            params.kvMode == .none && params.kvBits == nil
        if promptCacheRequested && !promptCacheCompatible {
            await self.log(.warn, "cache",
                "Prompt cache disabled for this request because KV cache compression is active; persistent compressed-cache restore is not beta-proven.")
        }
        let coordinator: CacheCoordinator? =
            (ProcessInfo.processInfo.environment["VMLX_DISABLE_CACHE_COORD"] == "1"
             || !promptCacheRequested
             || !promptCacheCompatible)
        ...
        let performResult: (
            ...
            try await container.perform { (ctx: ModelContext) in
                ...
                if ProcessInfo.processInfo.environment["VMLX_PROMPT_TRACE"] == "1" {
                    ...
                    var rendered = ctx.tokenizer.decode(tokenIds: tokenIds)
                    if rendered.count > 4_000 {
                        rendered = String(rendered.prefix(4_000)) + "\n...[truncated]"
                    }
                    let escaped = rendered
                        .replacingOccurrences(of: "\n", with: "\\n")
                    FileHandle.standardError.write(Data(
                        "[prompt-trace] tokens=\(tokenIds.count) text=\(escaped)\n".utf8))
                }
