...
// jang-runtime/Sources/JANGExpertLab/JANGExpertLab.swift. The Atlas, plan,
// evidence, and comparison behavior is preserved; only the inference seam is
// adapted to MLXStudioDomain.ModelInferenceProvider.
import Foundation
import CoreGraphics
import SQLite3
...
public struct ExpertPromptRun: Sendable {
    ...
    public let result: ExpertRunResult

    public init(prompt: ExpertPrompt, result: ExpertRunResult) {
        self.prompt = prompt
        self.result = result
    }
}

public enum ExpertPromptSuiteRunnerError: Error, Equatable, Sendable {
    case providerDidNotComplete(promptID: String)
}

/// Runs Expert Lab suites through the canonical inference boundary.
///
/// This replaces the source repository's direct ownership of `JANGKit.Model`.
/// Providers that implement expert tracing and masking consume the structured
/// metadata keys declared by `ExpertInferenceMetadataKey`.
public actor ExpertPromptSuiteRunner {
    private let provider: any ModelInferenceProvider
    private let artifactID: ModelArtifactID

    public init(provider: any ModelInferenceProvider, artifactID: ModelArtifactID) {
    ...
    public func run(
        ...
        config: ExpertSamplingConfiguration = ExpertSamplingConfiguration(maxTokens: 64),
        traceConfig: ExpertTraceConfiguration = ExpertTraceConfiguration()
        ...
        for prompt in suite.prompts {
            ...
            var metadata: [String: String] = [
                ExpertInferenceMetadataKey.traceEnabled: "true",
                ExpertInferenceMetadataKey.traceMaximumTokens: String(traceConfig.maxTraceTokens),
                ExpertInferenceMetadataKey.topK: String(promptConfig.topK),
            ]
            if let mask = traceConfig.mask {
                let encoder = JSONEncoder()
                encoder.outputFormatting = [.sortedKeys, .withoutEscapingSlashes]
                let data = try encoder.encode(mask)
                metadata[ExpertInferenceMetadataKey.maskJSON] = String(decoding: data, as: UTF8.self)
            }

            let request = GenerationRequest(
                artifactID: artifactID,
                messages: [GenerationMessage(role: .user, content: prompt.text)],
                configuration: GenerationConfiguration(
                    maximumTokenCount: max(1, promptConfig.maxTokens),
                    temperature: promptConfig.temperature,
                    topP: promptConfig.topP
                ),
                traceOptions: InferenceTraceOptions(capturesExpertRouting: true),
                metadata: metadata
            )

            var streamedTrace: [ExpertRoutingObservation] = []
            var completedResult: GenerationResult?
            for try await event in provider.events(for: request) {
                switch event {
                case .trace(let observation):
                    streamedTrace.append(observation)
            ...
            guard let completedResult else {
                throw ExpertPromptSuiteRunnerError.providerDidNotComplete(promptID: prompt.id)
            }
            let observations = streamedTrace.isEmpty
                ? (completedResult.trace?.expertRouting ?? [])
                : streamedTrace
            let limitedObservations = Array(observations.prefix(traceConfig.maxTraceTokens))
            let tokenTrace = limitedObservations.map {
                Self.routeRecord(from: $0, mask: traceConfig.mask)
            }
            let expertResult = Self.result(
                from: completedResult,
                tokenTrace: tokenTrace,
                observedTraceCount: observations.count,
                traceConfig: traceConfig
            )
            runs.append(ExpertPromptRun(prompt: prompt, result: expertResult))
        }
        return runs
    }

    private static func routeRecord(
        from observation: ExpertRoutingObservation,
        mask: ExpertMask?
    ) -> ExpertRouteRecord {
        let weights = observation.gateWeights.map(Float.init)
        let positiveTotal = observation.gateWeights.reduce(0) { total, weight in
            total + max(0, weight)
        }
        let entropy: Float? = positiveTotal > 0
            ? Float(-observation.gateWeights.reduce(0) { total, weight in
                let probability = max(0, weight) / positiveTotal
                return probability > 0 ? total + probability * log(probability) : total
            })
            : nil
        return ExpertRouteRecord(
            tokenIndex: observation.tokenIndex,
            layer: observation.layerIndex,
            selectedExperts: observation.selectedExpertIndices,
            scores: weights,
            disabledExperts: Array(mask?.disabledExperts(for: observation.layerIndex) ?? []).sorted(),
            effectiveTopK: observation.selectedExpertIndices.count,
            entropy: entropy
        )
    }
    ...
    private static func result(
        ...
        observedTraceCount: Int,
        traceConfig: ExpertTraceConfiguration
    ) -> ExpertRunResult {
        let traceMetadata = result.trace?.metadata ?? [:]
        var notes: [String] = []
        if tokenTrace.isEmpty {
            notes.append("Provider emitted no expert-routing observations.")
        }
        if observedTraceCount > tokenTrace.count {
            notes.append(
                "Expert-routing trace was capped at \(tokenTrace.count) of \(observedTraceCount) observations."
            )
        }
        if traceConfig.mask != nil,
           traceMetadata[ExpertInferenceMetadataKey.maskApplied] != "true" {
            notes.append("Provider did not attest that the expert mask was applied.")
        }

        return ExpertRunResult(
            text: result.text,
            tokens: result.metrics.generatedTokenCount,
            elapsedSeconds: result.metrics.totalDurationSeconds
                ?? result.metrics.generationDurationSeconds,
            tokensPerSecond: result.metrics.tokensPerSecond ?? 0,
            finishReason: Self.finishReason(from: result.finishReason),
            layerStats: Self.layerStats(from: tokenTrace),
            tokenTrace: traceConfig.emitTokenTrace ? tokenTrace : nil,
            runtimeInfo: ExpertModelRuntimeInfo(
                backend: traceMetadata[ExpertInferenceMetadataKey.backend] ?? "model_inference_provider",
                runtimeMode: traceMetadata[ExpertInferenceMetadataKey.runtimeMode] ?? "provider",
                deviceName: traceMetadata[ExpertInferenceMetadataKey.deviceName] ?? "unknown",
                metalEnabled: traceMetadata[ExpertInferenceMetadataKey.metalEnabled] == "true",
                sourceModelPath: traceMetadata[ExpertInferenceMetadataKey.sourceModelPath],
                hookedMOELayers: Int(traceMetadata[ExpertInferenceMetadataKey.hookedMOELayers] ?? ""),
                expectedMOELayers: Int(traceMetadata[ExpertInferenceMetadataKey.expectedMOELayers] ?? ""),
                hookCoverageComplete: Self.optionalBoolean(
                    traceMetadata[ExpertInferenceMetadataKey.hookCoverageComplete]
                ),
                maskApplied: Self.optionalBoolean(
                    traceMetadata[ExpertInferenceMetadataKey.maskApplied]
                ),
                disabledExpertCount: traceConfig.mask?.layers.values.reduce(0) { $0 + $1.count },
                topKOverride: traceConfig.mask?.topKOverride,
                notes: notes
            )
        )
    }
    ...
    private static func finishReason(
        from finishReason: GenerationFinishReason
    ) -> ExpertGenerationFinishReason {
        switch finishReason {
        case .completed:
            return .stop
        case .length:
    ...
    private static func optionalBoolean(_ value: String?) -> Bool? {
    ...
    private static func layerStats(from records: [ExpertRouteRecord]) -> [ExpertLayerStats] {
        Dictionary(grouping: records, by: \.layer)
            .map { layer, records in
                var hitCounts: [Int: Int] = [:]
                var probabilityMass: [Int: Float] = [:]
                for record in records {
                    for (offset, expert) in record.selectedExperts.enumerated() {
                        hitCounts[expert, default: 0] += 1
                        if record.scores.indices.contains(offset) {
                            probabilityMass[expert, default: 0] += record.scores[offset]
                        }
                    }
                }
                return ExpertLayerStats(
                    layer: layer,
                    tokenCount: Set(records.map(\.tokenIndex)).count,
                    hitCounts: hitCounts,
...
public enum ExpertArtifactWriter {
    ...
    private struct StoredGenerationRecord: Codable {
        ...
        let layerStats: [ExpertLayerStats]
