...
public func restoreLayerData(from blocks: [CacheBlock], into cache: [any KVCache]) -> Int {
    ...
    for (blockLayerIdx, cacheLayerIdx) in kvCacheIndices.enumerated() {
        ...
        if let simple = cache[cacheLayerIdx] as? KVCacheSimple {
            ...
            // 2026-05-01 (ref 1173822): route through restoreFromDecodedKV
            // (NOT `state =`). The previous `state =` path transitioned TQ
            // to fill phase with the lossy decoded float as the new
            // prefill, which then re-compressed at the next threshold
            // cross — compounding the lossy round each turn and producing
            // visible token degeneracy on multi-turn JANGTQ/MXFP4
            // conversations. The new method seats the decoded float
            // DIRECTLY as the compressed-phase prefix and stays in
            // `.compressed` so maybeQuantize skips it.
