...
public func generateTask(
    ...
    // Store prompt-only cache state before consuming the iterator. The
    // iterator has completed prefill at this point, but the decode loop has
    // not yet mutated its live KV cache with generated tokens. Storing later
    // captured post-generation state under a prompt-only key, which poisoned
    // the next prefix hit.
...
public func generateTokenTask(
    ...
    // Store prompt-only cache state before consuming the iterator. See the
    // text-generation twin above for the correctness rationale.
    let cacheStoreAction: (@Sendable () -> Void)? = {
        guard let coordinator = iterator.cacheCoordinator,
              !iterator.promptTokenIds.isEmpty else { return nil }
        let promptTokenIds = iterator.promptTokenIds
...
private func generateLoopTask<Handler: TokenLoopHandler>(
    ...
    let task = Task {
        let performIteration = {
            ...
            // Legacy hook for callers that explicitly need turn-end storage.
            // The main sync generate paths store prompt-only cache before
            // decode; running this after decode would serialize generated
            // token state under a prompt key.
