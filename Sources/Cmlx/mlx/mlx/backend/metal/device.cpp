...
bool env_enabled(const char* primary, const char* fallback = nullptr) {
  const char* value = std::getenv(primary);
  if ((!value || !value[0]) && fallback) {
...
std::string safe_library_name(const std::string& name) {
  std::string safe;
  safe.reserve(name.size());
  for (unsigned char c : name) {
    safe += (std::isalnum(c) || c == '_' || c == '-') ? char(c) : '_';
  }
  return safe.empty() ? "kernel" : safe;
}

std::filesystem::path external_jit_cache_dir() {
  if (const char* override_dir = std::getenv("VMLX_METAL_JIT_CACHE_DIR")) {
    if (override_dir[0]) {
      return override_dir;
    }
  }
  if (const char* override_dir = std::getenv("VMLINUX_METAL_JIT_CACHE_DIR")) {
    if (override_dir[0]) {
      return override_dir;
    }
  }
  ...
  return std::filesystem::temp_directory_path() / "vmlx-metal-jit";
}

std::string read_file_lossy(const std::filesystem::path& path) {
...
MTL::Library* build_library_with_external_metal(
    MTL::Device* device,
    const std::string& name,
  ...
  std::error_code ec;
  auto cache_dir = external_jit_cache_dir();
  std::filesystem::create_directories(cache_dir, ec);
  if (ec) {
    throw std::runtime_error(
        "[metal::Device] external Metal JIT cache unavailable at <" +
        cache_dir.string() + ">: " + ec.message() + "\n" + original_error);
  }

  const auto source_hash = std::hash<std::string>{}(source_string);
  const auto stem = safe_library_name(name) + "-" +
      std::to_string(source_string.size()) + "-" + std::to_string(source_hash);
  const auto metal_path = cache_dir / (stem + ".metal");
  const auto lib_path = cache_dir / (stem + ".metallib");
  const auto log_path =
      cache_dir / (stem + "-" + std::to_string(counter.fetch_add(1)) + ".log");

  if (std::filesystem::exists(lib_path, ec)) {
    auto [cached, error] = load_library_from_path(device, lib_path.c_str());
    if (cached) {
      if (env_enabled("VMLX_METAL_JIT_TRACE", "VMLINUX_METAL_JIT_TRACE")) {
        std::cerr << "[metal-jit] loaded external cache: " << lib_path
                  << std::endl;
      }
      return cached;
    }
  ...
  {
    std::ofstream out(metal_path);
    if (!out) {
      throw std::runtime_error(
          "[metal::Device] unable to write external Metal source <" +
          metal_path.string() + ">\n" + original_error);
    }
    out << source_string;
  }

  const std::string command = "xcrun -sdk macosx metal " +
      shell_quote(metal_path) + " -o " + shell_quote(lib_path) + " 2> " +
      shell_quote(log_path);

  if (env_enabled("VMLX_METAL_JIT_TRACE", "VMLINUX_METAL_JIT_TRACE")) {
    std::cerr << "[metal-jit] external compile: " << name << " -> "
              << lib_path << std::endl;
  }
  ...
  if (result != 0) {
    throw std::runtime_error(
        "[metal::Device] external Metal compile failed for " + name +
        "\ncommand: " + command + "\n" + read_file_lossy(log_path) + "\n" +
        original_error);
  }

  auto [lib, load_error] = load_library_from_path(device, lib_path.c_str());
  if (!lib) {
    std::string message = load_error
        ? load_error->localizedDescription()->utf8String()
        : "unknown load error";
    throw std::runtime_error(
        "[metal::Device] external Metal compile produced unloadable library <" +
        lib_path.string() + ">: " + message + "\n" + original_error);
  }
...
MTL::Library* Device::get_library(
  ...
  if (env_enabled("VMLX_METAL_JIT_TRACE", "VMLINUX_METAL_JIT_TRACE")) {
    std::cerr << "[metal-jit] building library: " << name << std::endl;
  }
  auto source = builder();
  MTL::Library* mtl_lib = nullptr;
  try {
    mtl_lib = build_library_(source);
  } catch (const std::exception& error) {
    if (!env_enabled(
            "VMLX_METAL_EXTERNAL_JIT", "VMLINUX_METAL_EXTERNAL_JIT")) {
      throw;
    }
    mtl_lib = build_library_with_external_metal(
        device_, name, source, error.what());
  }
