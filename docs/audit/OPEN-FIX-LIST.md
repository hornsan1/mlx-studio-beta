...
| # | Issue | Status | Section |
|---|-------|--------|---------|
| F4 | **Text selection** — users can't copy error messages, notifications, settings field labels, message bodies. SwiftUI needs `.textSelection(.enabled)` applied broadly, and modifier key drag should allow multi-region selection (macOS standard) | 🚧 sweep done | §357 app-wide sweep — scene-level blanket now on all 4 scenes (main window, Downloads, Settings, **MenuBarExtra tray added this pass**). Explicit per-view modifiers added to 38 high-value Text nodes across 16 files: error banners / failure text (BannerView toast, BenchmarkPanel, MCPPanel ×2, DownloadsWindow, ImageGallery banner, ValidatedField, APIScreen port errors, AdvancedServerCard TLS + apply status, ImageSettings warnings, HF token status, install-state labels), paths & model IDs (Models-list path + displayName, Hub repo row, install localPath, ModelDirectoriesPanel, CachePanel disk dir, SessionConfigForm read-only cache dir, Downloads repo + cache-path hint), endpoints (SessionCard host/host:port, APIScreen session host:port), diagnostics (shared value-row helpers serverChecklistRow / diag step / incidentFact, Studio job + log-line messages, CachePanel warm status, Setup status line). Deliberately skipped: button/toggle/picker labels, nav chrome, decorative captions, restart-required badges, NSAlert (AppKit, not selectable via SwiftUI). Still open: modifier-key drag multi-region selection (needs NSTextView-backed surface, out of scope for modifier sweep). |
