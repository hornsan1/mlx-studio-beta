...
| MAESTRO | No implementation at the three Phase 0 SHAs; paper provenance is Goel et al., arXiv:2607.08601v1, with no linked public reference implementation | Canonical experimental evidence adapter: `Sources/MLXStudioOptimization/MAESTROPruningStrategy.swift`; exact paper matrix only, external stationary-routing evidence, uniform per-layer survivor constraint, never default, and no implicit recovery |
...
- The canonical PR 12 MAESTRO boundary is explicitly **Global routing — Experimental** and is never automatically selected. It accepts precomputed stationary-distribution evidence because arXiv:2607.08601v1 links no public implementation; support is limited to the paper's `gpt_oss` and `qwen3_moe` families. Uniform per-layer removal, first-order Markov and whole-expert limitations, external recovery usage, one-shot retention, and post-recovery retention are all structured and reviewable.
