...
| REAP | `jang-tools/jang_tools/kimi_prune/jangreap.py`, `jang-tools/jang_tools/minimax_m3/reap_profile.py`, `jang-tools/jang_tools/minimax_m3/reap_select.py`, and DSV4 forward/ops under `jang-tools/jang_tools/dsv4/` | Specialized prototypes only; adapter support must be capability-gated |
| MAN | No implementation found | New production strategy required in PR 10 |
| MSAN | No implementation found | New production strategy required in PR 10 |
...
- `jang-runtime/Sources/JANGExpertLab/JANGExpertLab.swift` is a large combined domain/store/runner file and directly imports `JANG` and `JANGKit`.
