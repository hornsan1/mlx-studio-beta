...
| Pruning recommendation | Existing Expert Lab hit/mass heuristic; canonical MAN/MSAN activation-norm strategies; specialized REAP; JANG `recommend` conversion profile | `PruningStrategy` candidate generation plus measured evaluation | MAN/MSAN raw scores remain separate and use within-layer percentiles only for display/ranking; preserve provenance and require structural plus evaluation gates |
